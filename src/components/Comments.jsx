...
export default function Comments({ setISCommentPosted }) {
	...
	useEffect(() => {
		newsApi
			.get(`/articles/${article_id}/comments?p=${pageNumber}`)
			.then(({ data }) => {
	...
	function deleteComment(value) {
		...
		newsApi.delete(`/comments/${commentId}`).then((response) => {
			...
			{hasComments ? (
				...
				>
					...
					<p className="total">
						Showing comments:{" "}
						{`${pageNumber * 10 - 9} - ${
							pageNumber * 10 - 10 + commentsList.length
						} `}
					</p>
					<div id="commentsButtons">
						<div className="nextButton">
							{isFirstPage ? null : (
								<button onClick={previousPage}>Pevious comments</button>
							)}
						</div>

						<div className="previousButton">
							{isLastPage ? null : (
								<button onClick={nextPage}>Next comments</button>
							)}
						</div>
					</div>
				</CommentsButton>
